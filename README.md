...
力量训练中心使用独立入口和页面，与跑步、骑行教练界面分开；可直接访问 `#/strength-library`。页面包含 49 套预置训练计划和完整动作库：计划可按训练重点、水平及器械条件筛选，并显示动作顺序、组数、次数或时长、休息和 RPE；动作库支持动作名搜索，以及按训练部位、器械和训练方式筛选。
...
右上角设置可选择两种解读方式，点选后立即用于之后的 AI 复盘并保存在当前浏览器，无需再点击底部“保存”：

- **进阶训练（默认）**：心率部分只展示 Z1-Z5 的 bpm 范围、占比条和时间；之后依次展示可选的极光路段、续航/爬坡/冲刺训练刺激和下一次训练建议，不生成独立证据章节。
- **健康陪练**：沿用进阶训练的章节顺序，直接从强度分布开始，再展示可选心流路段、身体得到的锻炼和下一次建议；用“非常轻松、轻松有氧、中等强度、较高强度、很高强度”等日常语言解释五区，不添加开场总评或能力汇总段落。
...
上传 FIT、GPX 或 JSON 后，在“训练已导入”卡片点击“打开数据看板”，也可以从底部回放条再次打开。看板按跑步 / 骑行类型展示距离、时长、配速或速度、心率、功率、踏频、海拔和趋势图。FIT 如果记录了设备计时器，时长、平均速度和 AI 分析会使用排除暂停后的有效骑行时间；没有计时器的旧文件才回退到轨迹首尾时间差。
